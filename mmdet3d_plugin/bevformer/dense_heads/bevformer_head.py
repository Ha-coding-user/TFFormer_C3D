...
class BEVFormerHead(DETRHead):
    ...
    def forward(self, mlvl_feats, img_metas, img_inputs, prev_bev=None,  only_bev=True):
        """
        Descript:
            Forward Function
        Args:
            - mlvl_feats (list[Tensor]): Features from the upstream network     | [B, N, C, H', W']
            - img_metas (dict): meta data of img                                | {'pc_range', 'occ_size', 'sequence', 'frame_id', 'raw_img', 'stereo_depth', 'focal_length', 'baseline', 'img_shape', 'gt_depths'}
            - img_inputs (list): camera params                                  | {'rot', 'trans', 'intrin', 'post_rot', 'post_trans', 'bda_rot', 'cam2lidar', 'cam2world'}
            - prev_bev (Tensor): previous bev featur
            - only_bev (bool): only compute BEV features with encoder. 
        Returns (only_bev == True):
            - bev_features (Tensor): bev features                               | [B, H*W, C]
        ...
        object_query_embeds = self.query_embedding.weight.to(dtype) # Not need
        bev_queries = self.bev_embedding.weight.to(dtype)           # bev_queries: [H*W, C]

        bev_mask = torch.zeros((bs, self.bev_h, self.bev_w),
                               device=bev_queries.device).to(dtype) # bev_mask: [B, H, W]
        bev_pos = self.positional_encoding(bev_mask).to(dtype)      # bev_pos: [B, C, H, W]

        if only_bev:  # only use encoder to obtain BEV features, TODO: refine the workaround
            return self.transformer.get_bev_features(
                mlvl_feats,                             # mlvl_feats (list[Tensor]): multi-level image feature
                bev_queries,                            # bev_queries (Tensor): bev query
                self.bev_h,                             # self.bev_h (scalar): height of bev
                self.bev_w,                             # self.bev_w (scalalr): width of bev
                grid_length=(self.real_h / self.bev_h,  # one of grid's length (h, w)
                             self.real_w / self.bev_w),
                bev_pos=bev_pos,                        # bev_pos (Tensor): learnable positional encoding
                img_metas=img_metas,                    # img_metas (dict): meta data of img
                img_inputs=img_inputs,                  # img_inputs (list): camera params
                prev_bev=prev_bev,                      # prev_bev (Tensor?): previous bev features
            )
        else:
        ...
        # We do not need below code cause we just need bev features
